...
import {
  initManagedPolicy, managedDefaultsAsOptionsString,
} from './nassh_managed_policy.js';
import {
...
ConnectDialog.prototype.updateNasshOptionsPlaceholder_ = function() {
  ...
  if (!this.$f('nassh-options').value) {
    // Enterprise-managed default options take precedence over the hostname
    // heuristic below.
    const managed = managedDefaultsAsOptionsString();
    if (managed) {
      value = managed;
    } else {
